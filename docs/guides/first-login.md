...
1. Googleの画面が開き、アカウント選択画面が毎回表示されるので、手順0で許可してもらったアカウントを選びます。ブラウザに1つしかGoogleアカウントがない場合も、この画面で確認してから進みます。
...
## 別のGoogleアカウントに切り替えたいとき

ログイン中は「Googleでログイン」を押してもGoogleの画面を経由せず、そのままホームへ戻ります。別のアカウントで使いたいときは、次の順番で操作します。

1. アプリの設定画面（またはホームのheader）から「ログアウト」を押します。
2. ログイン画面で「Googleでログイン」を押します。
3. Googleのアカウント選択画面で、使いたいアカウントを選びます。一覧に無い場合は「別のアカウントを使用」から追加します。

選んだアカウントも許可リストに登録されている必要があります。登録されていない場合は「このGoogleアカウントは利用を許可されていません。」と表示されます。
