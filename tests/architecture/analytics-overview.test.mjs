...
test("分析の集計対象枠は折り返さず、選択欄は選択後に閉じる (AC-ANA-005-3, AC-ANA-005-4)", async () => {
  ...
  // 枠を複数行へ折り返さず、枠数にかかわらず等幅の列へ分ける（AC-CAL-004-3と同じ規則）
  const scopeNav = moduleCss.match(/\.analytics-scope-nav \{[^}]*\}/)?.[0];
  assert.ok(scopeNav);
  assert.doesNotMatch(scopeNav, /flex-wrap|grid-template-columns/);
  assert.match(scopeNav, /display:\s*grid;/);
  assert.match(scopeNav, /grid-auto-flow: column;/);
  assert.match(scopeNav, /grid-auto-columns: minmax\(0, 1fr\);/);
  assert.match(moduleCss, /\.analytics-member-picker \{[^}]*min-width: 0;/);
  assert.doesNotMatch(moduleCss, /has-member-slot/);
  assert.match(
    moduleCss,
    /\.analytics-member-picker > summary::after \{[^}]*content: "";/,
  );
  assert.doesNotMatch(overview, /has-member-slot/);
...
test("詳細分析の月入力欄はWebKitでも列幅に収まり、累積収支は純関数とServer Componentで描く (AC-ANA-009-7, AC-ANA-013-1〜3)", async () => {
  ...
  assert.match(
    moduleCss,
    /i\[data-trend-bar="savings"\]\[data-chart-bar="negative"\] \{[^}]*background:\s*var\(--chart-expense\);/,
  );
...
test("詳細分析はカテゴリ・月別推移を常時表示し、メンバー別・数値表を折りたたみ、貯金額を月別推移の系列にする (ANA-017, AC-ANA-013-2, AC-ANA-015-1, AC-ANA-017-1〜3)", async () => {
  ...
  assert.doesNotMatch(chart, /Math\.(max|min|round)|\/ *max|reduce\(/);
  // 貯金額の棒は塗りのtokenで描く。ダークでは文字用の--accentと値が分かれる (review: 2026-09-14-dark-theme)
  assert.match(
    moduleCss,
    /i\[data-trend-bar="savings"\] \{[^}]*background:\s*var\(--accent-surface\);/,
  );
});
