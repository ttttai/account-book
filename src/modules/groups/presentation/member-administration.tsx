...
function RoleChangePanel({
  ...
  return (
    <form action={formAction} className={styles["member-admin-panel"]}>
      ...
      <p className={styles["member-admin-confirm-text"]}>
        「{target.displayName}」（現在: {roleLabels[target.role]}）の権限を「
        {roleLabels[selectedRole]}」へ変更します。よろしいですか？
      </p>
      <div className={styles["member-admin-panel-actions"]}>
...
export function MemberAdministration({
  ...
  return (
    <div className={styles["member-administration"]}>
      <div className={styles["member-admin-triggers"]}>
        {assignableRoles.length > 0 && (
          <button
            aria-expanded={openPanel === "role"}
            className={styles["member-admin-trigger"]}
        ...
        {canRemove && (
          <button
            ...
            className={`${styles["member-admin-trigger"]} danger-text`}
            onClick={() =>
