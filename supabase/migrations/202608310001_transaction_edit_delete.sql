-- 取引の編集・削除command（TXN-008、TXN-009、TXN-012）
-- 行lockと楽観的ロック（expected version）で同時更新を検出する。
-- 削除は物理削除とし、復元機能は提供しない（R-050）。
...
-- 取引を楽観的ロック付きで物理削除する（存在しない対象への再送は状態を変えず成功する）
...
begin
  ...
  select transaction.id, transaction.version
  into target
  from public.transactions transaction
  where transaction.id = p_transaction_id
    and transaction.group_id = p_group_id
  ...
  -- 削除の再送（すでに存在しない対象）は冪等に成功として扱う (AC-TXN-009-4)
  ...
  -- 負担行と取引本体を同じtransactionで物理削除する (AC-TXN-009-1)
  delete from public.transaction_allocations
  where transaction_id = p_transaction_id
    and group_id = p_group_id;
