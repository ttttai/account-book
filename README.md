...
デプロイとDB変更の運用手順は[`docs/operations/`](docs/operations)（[デプロイ運用](docs/operations/deployment.md) / [DB変更の運用](docs/operations/database-changes.md) / [許可Googleアカウントの追加・削除](docs/operations/allowed-google-emails.md)）にまとめています。LINE週次レポート（段階導入中）は[導入ガイド](docs/operations/line-weekly-report.md)、LINEのメンションからの不具合Issue自動起票（段階導入中）は[導入ガイド](docs/operations/line-bug-report.md)を参照してください。
