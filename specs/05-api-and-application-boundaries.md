...
カレンダー（`getGroupCalendar`）と分析（`getAnalyticsOverview`・`getAnalyticsPeriodSummary`）は`listMonthlyTransactions`の結果へそれぞれの集計純関数を適用し、取引行の読み取りと定期取引の展開を独自に実装しない。履歴は絞り込みとcursor paginationのため取引queryを固有に持つが、認証・所属・表示名は共有境界を使う。予算、詳細分析など月次の支出・収入実績を必要とする画面機能は、この境界を再利用する。共有境界は呼び出しごとに認可を再確認し、cacheを持たない。
