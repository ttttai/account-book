...
- 2人グループでは集計対象が「グループ」「自分」「相手の表示名」の3枠で、選択欄を開かず相手の名前から直接切り替えると月間合計が`￥3,000`になる（`AC-CAL-004-1`・`AC-CAL-004-2`）。URLと選択枠の一致、月移動時の対象保持、320px・375px・1280pxで1行・44px以上・横scrollなし、「グループ」「自分」が等幅で3枠目がそれより広く2倍以内であること、320pxでも相手の表示名「E2E利用者B」が省略されず`title`に全文を持つことを確認する（`AC-CAL-004-3`）。招待前の自分だけのグループでは2枠とする。
