...
| `E2E-004` | 均等共有支出の登録とカレンダー・履歴の一致 | `TXN-001`、`TXN-005`〜`TXN-007`、`TXN-019`、`CAL-008`、`HIS-004`      |
| `E2E-005` | 支出の編集と削除                           | `TXN-008`、`TXN-009`、`TXN-012`、`TXN-019`                            |
| `E2E-006` | 日付選択の即時反映とURL同期                | `CAL-002`、`CAL-005`、`CAL-011`                                       |
| `E2E-007` | CSV出力                                    | `EXP-001`、`EXP-002`                                                  |
| `E2E-008` | 320pxでの横スクロール不発生                | `NFR-UI-002`                                                          |
| `E2E-009` | 固定費の登録と一覧表示                     | `REC-001`〜`REC-004`、`AC-REC-004-1`、`AC-REC-004-2`、`AC-REC-002-2`  |
| `E2E-010` | 詳細分析の期間統計・数値表・URL同期        | `ANA-006`〜`ANA-009`、`ANA-013`、`ANA-015`、`ANA-016`、`AC-ANA-012-3` |
...
- 2人グループでは集計対象が「グループ」「自分」「相手の表示名」の3枠で、選択欄を開かず相手の名前から直接切り替えると月間合計が`￥3,000`になる（`AC-CAL-004-1`・`AC-CAL-004-2`）。URLと選択枠の一致、月移動時の対象保持、320px・375px・1280pxで1行・44px以上・横scrollなし、3枠が等幅で、枠に収まらない表示名を末尾省略してもaccessibility nameへ全文が残ることを確認する（`AC-CAL-004-3`）。招待前の自分だけのグループでは2枠とする。
...
- 絞り込みsheetの「キーワード」にメモの一部（「均等」）を入れてEnterを押すと、ページ全体を再読み込みせずURLへ`q`が保存され、同じ取引が残る。一致しない語（「該当なし」）では「「該当なし」に一致する取引はありません。」が表示され、適用中条件の「キーワード「該当なし」」の「解除」で一覧が戻る（`AC-HIS-009-2`〜`AC-HIS-009-4`）。
- 日別取引の行に両メンバーの表示名が表示され、「内訳」の見出し語・各メンバーの金額・「支払者」・行内の「編集」リンクが表示されない（`AC-CAL-017-1`）。
- 支出を保存してカレンダーへ遷移した直後、「保存結果の通知」領域に「支出を登録しました」と、当日の「M/D」・「食費」・「￥6,000」を並べた説明が表示され、通知に「支払者」「負担」「内訳」が含まれない（`AC-TXN-019-1`、`AC-TXN-019-3`）。

### E2E-005 支出の編集と削除

- 履歴または日別取引の行をタップして編集画面を開き、金額を変更して保存すると、遷移元へ戻りカレンダー合計が更新される（`AC-CAL-017-2`）。遷移後の通知に「支出を更新しました」と更新後の金額`￥8,000`が表示される（`AC-TXN-019-2`）。
- 保存時に現在の`version`を`expectedVersion`として送る。
- 削除は確認を経てから実行し、成功後はカレンダー合計と履歴から即時に除外される。遷移後の通知に「支出を削除しました」と削除前の金額`￥8,000`が表示される（`AC-TXN-019-2`）。
- 通知は本体のタップで即時に消える（`AC-TXN-019-4`）。
...
- 固定費が0件の新規グループで固定費画面を開くと、headerに「ホームへ戻る」のリンクが無く、一覧の位置に「固定費はまだありません」と作成フォームへ移動するリンク「固定費を追加する」が表示される。リンクを押すと作成フォームへ移動し、登録後は空状態が消える（`AC-REC-004-2`、`AC-NAV-001-4`）。
...
- 支出を登録したグループで詳細分析を開き、「6か月の支出」の期間指標と月別数値表が同じ金額を示す（`AC-ANA-008-5`）。
- メンバー別の支出額・受取額を別の見出しで確認でき、支払額の列は表示されない（`AC-TXN-018-4`）。
- 320px・375pxではメンバー別表の各セルに項目名が表示され、1280pxでは列見出しを使いセル内の補助ラベルが隠れる。月別数値表は全幅で列見出しが見え、セル内の補助ラベルを持たない。全幅で数値と見出しの対応を保持する（`AC-ANA-009-6`）。
- 3か月presetの適用後は終了月をURLへ保持し、見出しが「3か月の支出」になり、月別数値表が3行になる。「表示する」ボタンは存在しない。
- 集計対象を「自分」へ変えると、ページ全体を再読み込みせずにURLへ`scope=self`が保存され、メンバー別表が消える。タップ前に`window`へ置いた印がタップ後も残ることで全画面再読み込みが無いことを確認する（`AC-ANA-016-1`、`AC-ANA-016-2`）。
- セクションは表示条件、期間指標、支出カテゴリ構成、月別推移、「メンバー別」（開閉ボタンが`aria-expanded="true"`）、「月別の正確な数値」（`aria-expanded="false"`、desktop projectでは開閉ボタンが無く常に表示）の順で、独立した「貯金額の推移」regionは無い（`AC-ANA-017-1`〜`AC-ANA-017-3`）。
- 月別推移で「貯金額」を選ぶと、同じ領域に注記と期間末の累積収支が表示され、月別数値表の累積収支列が最終月で期間の収支と同じ符号付き金額を示す（`AC-ANA-013-1`〜`AC-ANA-013-3`）。
