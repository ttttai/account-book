...
- 2人グループでは集計対象が「グループ」「自分」「相手の表示名」の3枠で、選択欄を開かず相手の名前から直接切り替えると月間合計が`￥3,000`になる（`AC-CAL-004-1`・`AC-CAL-004-2`）。URLと選択枠の一致、月移動時の対象保持、320px・375px・1280pxで1行・44px以上・横scrollなしを確認する。招待前の自分だけのグループでは2枠とする。
- 集計領域に支払額を表示しない（`CAL-010`）。
- 履歴の「自分の支出」に、主金額（「〇〇の支出」）`￥3,000`・取引全体`￥6,000`の取引が1件表示され、行に「支払者」が表示されない。
