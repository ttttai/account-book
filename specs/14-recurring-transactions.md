...
状態: 承認済み（review: 2026-09-19-recurring-empty-state）
...
標準URL: `/groups/{groupId}/recurring-transactions`。設定ハブから遷移する。共通ナビゲーションの現在地は「設定」とする。headerはグループ名と画面名だけとし、「ホームへ戻る」など共通ナビゲーションと重複する画面間移動を置かない（`03-screen-specification.md` §2、`AC-NAV-001-4`、`AC-REC-004-2`）。
...
- 0件のときは、一覧の位置に「固定費はまだありません」と、何を登録できるかが分かる1文の説明を空状態として表示する。owner/adminには作成フォームへ移動するリンク「固定費を追加する」を空状態の中へ置き、memberには置かない（`AC-REC-004-2`）。1件以上あるときは空状態を出さず、一覧を先頭に置く順序を保つ。
...
- 金額欄は入力中も取引入力（`AC-TXN-014-10`）と同じ3桁区切りで表示する（`128,000`）。表示用の`input`は`name`を持たず、送信する`amountMinor`は区切りなしの整数をhidden inputで送る。物理キーボードや貼り付けで入った`,`は無視する。整形関数は`transactions`モジュールの公開エントリーポイント経由で再利用し、固定費側へ複製しない（`AC-REC-005-5`）。
...
- 展開取引には編集・削除の操作を出さず、行全体のリンクで固定費画面へ誘導する（`AC-CAL-017-2`）。
