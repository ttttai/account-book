...
初回スコープは**固定額・月次**だけとする。job、scheduler、queue、retry、backfillを使わず、月ごとの通常取引レコードも自動生成しない。定期取引は「設定」として保存し、読み取り時に対象月の同じ日付へ展開して表示・集計する。

## 2. 用語

| 用語       | 意味                                                                        |
| ---------- | --------------------------------------------------------------------------- |
| 定期取引   | 毎月同じ日付・同じ金額で発生する取引の設定。`recurring_transactions`の1行。 |
| 展開       | 定期取引設定から、対象月の日付を持つ表示・集計用の擬似取引を作ること。      |
| 単発取引   | `transactions`に実在する通常の取引。                                        |
| occurrence | 展開結果の1件。DBへ保存せず、読み取りのたびに作る。                         |

## 3. 機能要件

- `REC-001` ownerまたはadminは、固定額の月次定期取引（支出・収入）を作成できる。
- `REC-002` 定期取引は、毎月同じ日付（1〜28日）、開始月、任意の終了月を持つ。
- `REC-003` 支出の定期取引はアクティブな支払者と負担額（1人・均等・カスタム）を持ち、負担額合計を金額と一致させる。収入はアクティブな受取者を持ち、負担額を持たない。
- `REC-004` ownerまたはadminは定期取引を編集・終了でき、memberは閲覧だけできる。
- `REC-005` 定期取引は対象月へ展開し、ホームカレンダーの月間集計・日別表示へ単発取引と重複せずに含める。
- `REC-006` 展開は開始月から終了月（指定時）までに限り、対象期間外の月へ表示・集計しない。
- `REC-007` 定期取引の作成・編集・終了は、確定済みの単発取引を変更しない。
- `REC-008` 定期取引の表示・集計にjob、scheduler、queue、retry、occurrence保存テーブルを使わない。
- `REC-009` 展開された取引と単発取引を、画面上で識別できる。
- `REC-010` 定期取引の金額は、取引入力と同じ画面内テンキーで入力し、OSの仮想キーボードによって下部ナビゲーションや入力欄の配置を崩さない。
...
- 対象月`YYYY-MM`に対し、`start_month <= 対象月`かつ（`end_month is null`または`対象月 <= end_month`）の定期取引を展開する。
...
- 定期取引の金額・日付・負担を変更すると、過去月の展開結果も新しい設定で計算される。過去月の値を固定する必要が生じた場合は、その月の単発取引として登録する。確定値の保持は初回スコープに含めない。
- 定期取引を「終了」すると`end_month`が設定され、翌月以降は展開しない。設定自体は削除せず一覧へ残す。
...
- 終了済み（`end_month`が過去月）の定期取引は「終了」と分かる表示にし、一覧の末尾へ置く。
- memberには作成・編集・終了の操作を表示しない。
- 0件のときは、何を登録できるかが分かる空状態を表示する。

### 作成・編集

- 入力順: 種別、名称、金額、毎月の日付、開始月、終了月（任意）、カテゴリ、支払者または受取者、支出は負担方法、メモ。
...
- 支出の負担方法は取引入力と同じ「1人・均等・カスタム」を使い、初期選択は「1人」とする。
...
- 日別取引sheetでは、展開取引に「定期」と分かるlabelを付けて単発取引と区別する（`REC-009`）。
- 展開取引には編集・削除の操作を出さず、定期取引画面での設定変更へ誘導する。
...
- 履歴一覧・CSV出力への展開（別段階で追加する。初回は定期取引画面とホームカレンダーだけへ表示する）
