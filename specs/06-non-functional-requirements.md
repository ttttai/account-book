...
- `NFR-PERF-008` 詳細分析の表示条件（期間・集計対象・メンバー）の変更では、ページ全体の再読み込みやServer Componentの再取得を行わず、認可を毎回確認する薄いServer Actionで集計結果のDTOだけを取得する。header・下部ナビゲーション・表示条件は同じ要素を維持し、集計結果の領域だけを待機表示にして表示中の値を薄く残す。
