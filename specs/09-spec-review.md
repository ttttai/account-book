...
バージョン0.2.22の自動文書検査では、要件ID 110件、明示的な受け入れ条件ID 123件が一意であり、重複宣言はなかった。過去版のレビューに記載した要件ID件数には集計誤りがあったため、0.2.21で宣言行を再集計して訂正した。0.2.22では`NFR-PWA-001`〜`NFR-PWA-005`の5件を追加した。
