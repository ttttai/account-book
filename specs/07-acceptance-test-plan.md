...
- LINE不具合報告のメンション抽出（`isSelf`のmentioneeだけを対象、`@All`のみ・非text・standby modeの除外、複数メンション範囲の除去と空本文の判定）、Issueタイトルの60コードポイント切り詰め、本文のfence長（backtick連続より長い）と未記入欄、報告元にuserId・groupIdを含めないこと、返信文（成功・失敗・空・userId応答）（`AC-LBR-001-1`〜`AC-LBR-005-1`、`AC-LBR-011-1`）
- LINE不具合報告の環境変数の検証（リポジトリ形式、許可userIdの形式・重複・空で全体を無効にすること）（`AC-LBR-010-1`）
...
LINE不具合報告の起票フローについて、DB・GitHub・LINEの境界をmockした実行testで次を確認する。

- 許可外userIdでは起票記録・GitHub・返信を呼ばず、結果に件数だけを持つこと
- 1対1トーク・未連携グループのメンションでは起票記録を確保しないこと
- 同じ`message.id`の2回目は起票記録の確保が`false`となりGitHubを呼ばないこと
- GitHub失敗・タイムアウト時に記録を返上し失敗文を返信し、例外を投げないこと。返信失敗も例外にしないこと
- 成功時にIssue番号を記録し、URLを含む返信を送ること。Push APIを呼ばないこと
- 1対1トークの`ID`で送信者自身のuserIdだけを返信すること
...
- `line_notifier`は`app_private.line_issue_reports`を直接参照できず、起票記録の確保は同一`message.id`の2回目に`false`を返し、返上後は再確保でき、Issue番号を記録した完了済みの行は返上で削除されない。不正な`message.id`と存在しないグループは拒否される（`AC-LBR-007-1`）。
...
20. 概要分析のカテゴリ上位5件と「その他のカテゴリ」の合計が期間支出と一致し、円グラフから棒グラフへ切り替えても一覧の値と順序が変わらないことを確認する。
21. CSVを出力する。
...
- 取引入力のテンキーの「閉じる」キーを320px・375pxで押せ、押すと数字キーが消えて保存が全幅になり、金額と再開の説明が残る。キーの追加でドックが高くならず、00・0・+と重ならない（`AC-TXN-014-8`、`AC-TXN-014-9`）。
