...
- 共有データ反映の方針（自動反映画面のpathname判定、失敗回数に応じた次回間隔のbackoffと上限、表示復帰時の再確認判定、入力欄focus中の保留判定）
- 変更tokenの決定性（同じ集約値から同じtoken、行数・`updated_at`のいずれかの変化でtokenが変わる、tokenから集約値を復元できない）
...
- 変更確認query（`getGroupChangeToken`）が未認証・非メンバー・不正IDへtokenを返さず、取引行を読まないこと。変更確認Route Handlerが401・404とJSON error codeを`no-store`で返し、redirectしないこと
- 変更確認componentが、自動反映画面だけで確認を行い、非表示中は要求を送らず、表示復帰で直ちに確認し、tokenが変わったときだけ`router.refresh()`を呼び、入力欄focus中は保留し、通信失敗でbackoffし、401・404で停止すること
- 履歴一覧が変更token・先頭ページ・次cursorで再取得を識別し、tokenが変わったときは先頭ページに変化がなくても追加ページと古い追加読み込み応答を破棄すること。同じtoken・内容のServer Action同梱RSCでは追加行を維持すること
...
25. 一方のメンバーがホームを開いたまま、他方が支出を登録すると、閲覧側が再読み込みや操作をしなくても60秒以内に月間合計と日別金額が更新され、その間にroute-levelのskeletonが表示されず、URLと選択状態が維持されることを確認する。
...
- ホームを開いたまま別メンバーが支出を登録すると、操作なしで月間合計と日別金額が更新され、その間にカレンダーがskeletonへ置き換わらず、選択日・集計対象・スクロール位置が維持される。取引入力画面では自動反映が起きず、入力中の金額が消えない。
