...
- 横スワイプの月移動判定（カレンダー幅の20%以上かつ48px以上、垂直移動量の1.5倍以上）が純関数で決まり、閾値未満・縦方向主体・pointer cancelでは月移動せず、日付セルのタップによる即時反映を妨げないこと。pointer captureは水平dragが始まってから取り、pointerdown時点では取らない（clickの発火先が判定領域へ変わり日付リンクの選択が失われるため）こと
- 横スワイプ中はカレンダーが水平方向へ追従して方向が表示され、閾値未満・縦方向主体・pointer cancelでは原点へ戻り、成立時は移動先方向へ送り出されること。動きを減らす設定では過度な移動がないこと
...
- カレンダーの横スワイプ中に追従表示と方向が分かり、月が移動する。閾値未満では元の位置へ戻り、縦スクロールおよびiOS Safariの戻る・進むジェスチャーと競合しない。
