...
- 通知専用ロール`line_notifier`はテーブルの直接参照と他機能のDB関数を実行できず、通知用関数だけを実行できる。集計元の関数は指定グループ・月範囲の未削除取引と定期取引の展開条件、対象月の適用予算改定だけを返し、個人名・メモ・支払者・負担者を返さない。送信枠は同一グループ・同一週で2回目に`false`を返し、返上後は再確保できる。
