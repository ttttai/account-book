...
- 詳細分析の累積収支（開始直前0円、0円月の引き継ぎ、負値、最終月と期間収支の一致）と折れ線グラフの座標・0円基準線・`aria-hidden`（`AC-ANA-013-1`〜`AC-ANA-013-3`）
...
- 詳細分析の貯金額の推移で、折れ線・0円基準線・最大値・最小値・開始月・終了月・注記が横scrollなく表示され、数値表の累積収支と一致する（`AC-ANA-013-2`、`AC-ANA-013-3`）。
