...
- 定期取引の作成・編集フォームの金額欄がOSの仮想キーボードを開かず、取引入力と共有する画面内テンキーで値を組み立て、金額欄以外へのfocusで閉じても入力値を保持し、テンキーを開いたときにキーが下部ナビゲーションへ隠れない位置へ移動すること
...
- 取引入力、定期取引、予算の金額欄でOSの仮想キーボードが開かず、画面内テンキーで金額を入力できる。
