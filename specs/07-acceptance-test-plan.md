...
- ホームと概要分析の集計対象が3枠のとき「グループ」「自分」を`4.5rem`下限の等幅、3枠目を残り幅（余白が許せば2倍）で描き、選択欄が名前部分と印（▾）を分けて名前だけを省略すること。320pxで「E2E利用者B」が省略されないことを実画面（E2E）で、幅比・省略規則・`title`をCSS構造testとcomponent testで確認する（`AC-CAL-004-3`）
