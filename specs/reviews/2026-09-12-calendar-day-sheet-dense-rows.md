...
architecture 193件、単体・component 991件（新規: 純関数test 8件、日別sheetのcomponent test 7件の追加・1件の更新）、lint、format、型検査、本番buildが成功。`E2E-004`は日別取引の「内訳」と各人`￥3,000`の断言を両メンバーの表示名・「内訳」「￥3,000」「編集」リンクの不在・行リンクのアクセシブル名へ、`E2E-005`は日別取引の「編集」clickを行リンク（「食費 ￥…」）のclickへ更新した。E2EはPRのCIで実行する。
