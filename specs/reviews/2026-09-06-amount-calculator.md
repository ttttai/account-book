...
- 共有部品`AmountKeypad`は`calculator`propを任意指定で受け取り、指定時だけ演算子列と=を描画する。定期取引・予算フォームは指定しないため表示・挙動を変えない。両フォームへの展開は`08-decisions-and-deferred-scope.md`の延期機能へ記録した。
...
- 定期取引・予算のarchitecture test（`.keypad`のPC幅ルールの限定、`AmountKeypad`の共有）は演算子列を条件付きで描画する構成のまま満たせる。

## MVP範囲確認

- 対象は取引入力のみとし、定期取引・予算は延期機能へ記録した。括弧、演算子の優先順位、履歴、小数、パーセント、メモリ機能は含めない。新しい依存packageやmigrationは追加しない。
...
- 定期取引・予算のテンキー（`calculator`未指定）は表示・component testともに変更なし。
