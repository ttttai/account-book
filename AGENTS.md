...
- 異なる機能や独立した作業を並列で開発する場合は、現在のローカル作業と変更が混ざらないよう、作業ごとに専用のGit worktreeとブランチを作成する。
- 1つのworktreeは1つのまとまった成果、ブランチ、PRに対応させる。同じブランチを複数のworktreeで同時に使用しない。
- worktreeは、対象仕様が承認され、受け入れ条件に対応するテストが作成または更新された時点の共通基点から作成する。worktreeを使う場合も「最優先の開発手順」の順序を変更しない。
...
- PRのmergeおよびauto-mergeの設定は行わない。差分をユーザーが確認し、手動でmergeする。
